...
define([
    ...
    function pivot(options, element) {
        ...
        function setupGestures() {
            pivotEl = ko.virtualElements.firstChild(element).nextSibling;
            container = $('.pivot-content >ul', pivotEl);
            //options.items(items);

            setTimeout(function () {
                showItem(1, true);

                return new Hammer(pivotEl, { drag_lock_to_axis: true })
    ...
    function wrapValueAccessor(valueAccessor, element) {
        return function () {
            var options = valueAccessor(),
                mypivot = pivot(options, element);

            return {
                name: 'pivot_template',
                data: mypivot,
                afterRender: function () {
                    mypivot.setupGestures();
                }
            };
        };
    }
